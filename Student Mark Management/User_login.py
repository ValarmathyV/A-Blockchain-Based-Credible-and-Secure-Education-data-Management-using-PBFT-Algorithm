    ...
    def register():
        if reg_entry01.get() != '' and reg_entry02.get() != '' and reg_entry03.get() != '':
            if reg_entry03.get() == reg_entry04.get():
                cur.execute("insert into userregister(Name,Email,Password,Status) values(%s,%s,%s,'Not Approved')",(reg_entry01.get(),reg_entry02.get(), reg_entry03.get() ))
                con.commit()
                        ...
                        for j in range(len(s)):
                            if s[j] == 'Verified':
                                ...
                                def approve(x1 = s[j-18], x2 = s[j-17], x3 = s[j-16], x4 = s[j-15], x5= s[j-14], x6 = s[j-13], x7 = s[j-12], x8 = s[j-11], x9 = s[j-10], x10 = s[j-9], x11 = s[j-8], x12 = s[j-7], x13 = s[j-6], x14 = s[j-5], x15 = s[j-4], x16 = s[j-3]):
                                    ...
                                    l18 =tk.Label(admin, text=' SEMESTER', font=('times new roman', 18, 'bold'), bg='WHITE',
                                                fg='black', )
                                    l18.place(x=200, y=50, width=300)

                                    l19 =tk.Label(admin, text=' MODEL EXAM', font=('times new roman', 18, 'bold'), bg='WHITE',
                                                fg='black', )
